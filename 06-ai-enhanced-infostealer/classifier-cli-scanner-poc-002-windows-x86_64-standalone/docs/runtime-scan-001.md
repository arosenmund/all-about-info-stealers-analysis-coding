...
`distribution-001` supplies the DirectML-based Windows x86_64 package, while
`distribution-002` supplies a CPU-only standalone Windows x86_64 package with
no DirectML or Visual C++ redistributable dependency. Native Windows execution
remains unverified for both. These are intended invocation contracts, not
cross-platform qualification claims. See
[`distribution-002`](distribution-002.md) for standalone prerequisites and
verification.
