"""Aggregate-only Phase 2 comparison of frozen CNN and n-gram configurations."""
...
USAGE = "Usage: phase02-compare-cnn --root <explicit-corpus-directory>"
...
def run(arguments: Sequence[str], stdout: TextIO, stderr: TextIO) -> int:
    """Compare only frozen models on the renewed development allocation."""
    ...
    try:
        ...
        report = compare_cnn_to_baseline(prepared, allocation)
        ...
        stderr.write("CNN comparison evaluation preparation could not be completed\n")
        return 1
    except CnnBaselineComparisonContractError:
        stderr.write("frozen CNN comparison could not be completed\n")
        return 1
