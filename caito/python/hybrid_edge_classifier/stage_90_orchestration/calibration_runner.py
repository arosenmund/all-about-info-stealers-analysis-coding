"""Redacted command boundary for the Python-only `calibration-001` POC."""
...
from phase_01_baseline import BaselineContractError, build_baseline_dataset
from phase_01_evaluation import CalibrationContractError, fit_calibration_and_evaluate
...
USAGE = "Usage: phase01-calibration --root <explicit-corpus-directory>"
...
def run(arguments: Sequence[str], stdout: TextIO, stderr: TextIO) -> int:
    """Fit calibration/policy evidence only against a labelled explicit root."""
    ...
    try:
        ...
        report = fit_calibration_and_evaluate(dataset)
        ...
        if error.code == "cross_label_conflict":
            stderr.write("calibration requires cross-label duplicate review\n")
        else:
            stderr.write("calibration split planning could not be completed\n")
        return 1
    except (BaselineContractError, CalibrationContractError):
        stderr.write("calibration evaluation could not be completed\n")
        return 1
